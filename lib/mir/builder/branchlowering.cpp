...
std::list<std::shared_ptr<Instruction>> InstLowering::retLower(const std::shared_ptr<IR::RETInst> &ret) {
    ...
    if (retType != IR::IRBTYPE::FLOAT) {
        ...
        auto mov = std::make_shared<movInst>(SourceOperandType::i, operlower.getPreColored(CoreRegister::r0), retVal);
        ...
        if (auto ret_const = std::dynamic_pointer_cast<IR::ConstantFloat>(ret->getRetVal())) {
            ...
            auto vmov =
                std::make_shared<Vmov>(SourceOperandType::r, operlower.getPreColored(FPURegister::s0), retVal, pair);
