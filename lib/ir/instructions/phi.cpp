...
namespace IR
{
    ...
    std::vector<std::shared_ptr<PhiOperand>> PHIInst::getPhiOpers() const {
    ...
    PhiOperand::PhiOperand(const std::shared_ptr<Value> &_value, const std::shared_ptr<BasicBlock> &_block)
        : User("__phi_operand", makeBType(IRBTYPE::UNDEFINED), ValueTrait::PHI_OPERAND) {
        addOperand(_value);
        addOperand(_block);
    }

    std::shared_ptr<Value> PhiOperand::getValue() const {
        return getOperands().front()->getValue();
    }

    std::shared_ptr<BasicBlock> PhiOperand::getBlock() const {
        return std::dynamic_pointer_cast<BasicBlock>(getOperands().back()->getValue());
    }

    PHIInst * PhiOperand::getPhi() const {
