...
public:
    ...
    explicit LatticeVal(Type type_) : type(type_) { Err::gassert(type != Type::CONSTANT_RANGE); }
    ...
    bool isZero() const {
        ...
        if (auto exact = rng.getExactConstant()) {
            return exact == false || exact == '\0' || exact == 0 || exact == 0.0f;
        }
        return false;
    }
};
...
public:
    ...
    LatticeVal merge(LatticeVal lhs, LatticeVal rhs) const override {
        ...
        if (lhs.isConstantRange() && rhs.isConstantRange() && lhs.getConstantRange() == rhs.getConstantRange()) {
            return lhs;
        }
        ...
        if (auto bin = inst->as<BinaryInst>()) {
            ...
            case OP::FADD:
                if (lhs.isConstantRange() && rhs.isConstantRange())
                    changes[inst].setConstantRange(lhs.getConstantRange() + rhs.getConstantRange());
            ...
            if (lhs.isConstantRange() && rhs.isConstantRange()) {
                ...
                changes[inst] = LatticeInfo::NAC;
        } else if (auto fti = inst->as<FPTOSIInst>()) {
            auto val = solver.getVal(LatticeInfo::getKeyFromValue(fti->getOVal()));
            if (val.isConstantRange()) {
                ...
                changes[inst] = LatticeInfo::NAC;
        } else if (auto itf = inst->as<SITOFPInst>()) {
            ...
            if (val.isConstantRange()) {
                const auto &rng = val.getConstantRange();
                switch (zext->getOType()->as<BType>()->getInner()) {
                case IRBTYPE::I1:
                ...
                changes[inst] = LatticeInfo::NAC;
        } else if (auto alloca = inst->as<ALLOCAInst>()) {
            changes[inst] = LatticeInfo::NAC;
