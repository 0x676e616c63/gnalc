...
using ElisionCandidates = std::unordered_map<Value *, MemoryInfo>;
struct ElisionVisitor : ContextVisitor {
    ElisionCandidates* candidates;
    LAAResult* laa_res;
    ...
    void visit(Context ctx, Instruction &inst) override {
        ...
        // If there are untracked LOADInst, give up. Since we need to rewrite every load to apply mask.
            ...
            if (aa->isArray()) {
                (*candidates)[aa->array().base].loads.emplace_back(MemoryInfo::LoadInfo{
                    load,
                    ctx.iList(),
                    ctx.iter
                });
    ...
    ElisionVisitor(ElisionCandidates* candidates_, LAAResult* laa_res_)
    : candidates(candidates_), laa_res(laa_res_) {}
};
...
PM::PreservedAnalyses CopyElisionPass::run(LinearFunction &function, LFAM &lfam) {
    ...
    ElisionCandidates candidates;
    ElisionVisitor visitor(&candidates, &laa_res);
    ...
    for (const auto &[dest_mem, mem_info] : candidates) {
        ...
        // the destination view is simply a reordered or copied version of
        // the source.
        // Here we compute a shuffle mask and try to replace subsequent destination accesses
        // with direct source accesses, thus eliminating the temporary copy/shuffle.
        // For example, accesses a[i][j] and a[j][i] differ only by a swap;
        // the mask {1, 0} converts one accesses into the other without any explicit copying.
        ...
        for (const auto& [info, access] : masked_loads) {
            ...
            for (const auto& expr : access) {
                ...
                if (coe != 1) {
                    auto bin = std::make_shared<BinaryInst>("%cpelim." + std::to_string(name_cnt++),
                        OP::MUL, subscript, function.getConst(coe));
                    info.ilist->insert(info.iter, bin);
                    subscript = bin;
                }
                auto gep = std::make_shared<GEPInst>("%cpelim." + std::to_string(name_cnt++),
                    base, i32_zero, subscript);
