...
// <alignment>] [, addrspace(<num>)] ty 是 numelements 的类型
//
// 类型一：%1 = alloca [2 x i32], align 4 //
// 静态分配，在函数开头，用于确定大小的数组 类型二：%array2 = alloca i32, i32 %1
// // 动态分配，需保存栈帧？
//
...
public:
    ALLOCAInst(NameRef name, std::shared_ptr<Type> btype,
               int _align = 4); // FOR STATIC
    // ALLOCAInst(NameRef name, std::shared_ptr<Type> btype,
    // std::shared_ptr<Value> num_elements, int _align = 4); // FOR DYNAMIC

    std::shared_ptr<Type> getBaseType() const;
    bool isStatic() const;
    // std::vector<int> getStaticArraySize() const;
    bool isArray() const;
    // std::shared_ptr<Value> getNumElements() const; // 修改使用 User基类
    // 的方法
    int getAlign() const;
