...
private:
    ...
    SCEVExprType type;
public:
    bool operator==(const SCEVExpr& other) const {
...
enum class TRECType {
    AddRec, Expr, Undefined, Untracked
};
// Tree of Recurrences
class TREC {
...
public:
    explicit TREC(TRECType type_)
        : type(type_) {
        Err::gassert(type != TRECType::Expr);
    }
    explicit TREC(SCEVExpr* scev_expr)
        : value(scev_expr), type(TRECType::Expr) {}

    explicit TREC(AddRec rec)
        : value(rec), type(TRECType::AddRec) {}

    static TREC undef() {
        return TREC(TRECType::Undefined);
    }
    static TREC untracked() {
        return TREC(TRECType::Untracked);
    }
    static TREC expr(SCEVExpr* i) {
        return TREC(i);
    }
...
class SCEVHandle {
    friend class TREC;
public:
    SCEVHandle(Function* func) : function(func) {
