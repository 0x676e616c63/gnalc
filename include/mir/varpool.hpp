...
private:
    ...
    std::unordered_map<ConstObj, std::shared_ptr<BindOnVirOP>, LoadMapHash> load_map;
...
public:
    ...
    std::pair<std::shared_ptr<BindOnVirOP>, std::shared_ptr<ConstObj>> getLoaded(const T_variant &literal) {
        unsigned int cnt = 0;
        for (const auto &pair : load_map) {
            if (literal ==
                std::get<std::remove_cv_t<std::remove_reference_t<decltype(literal)>>>(pair.first.getLiteral()))
    ...
    std::shared_ptr<BindOnVirOP> getLoaded(const ConstObj &obj) { return load_map[obj]; }
