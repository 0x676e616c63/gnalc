...
关于 For 的 Alias Analysis。   
我们将 Memory Access 分为 Scalar Access 和 Array Access。其中 Array Access 如下，
...
直观上来讲，如果把所有的不等式约束抽象成一个 N 维空间上的多面体，Omega Test 利用 Fourier-Motzkin 消去法求其在 N - 1
维空间上的投影。分析投影与投影变量的上下界以推断
原多面体内是否可能存在整数点。

![omega_test](/docs/images/omega_test.png)
...
尝试将 While 循环转换为 For 循环，此时归纳变量变为 `IndVar`，它与 Phi 节点类似，在 lower 到 IR 时直接替换为 Header
...
将 For 内数组的复制，转置等等传播到使用点，消除不必要中间数组复制。

#### Relayout

对于访问模式局部性差的循环，如果 LoopInterchange 不能优化访问模式，则 Relayout 尝试将数组转置，改变数据的排布，从而提升访问的局部性。
...
IR 是 SIR 的后继，我们使用的 IR 是 [LLVM IR](https://llvm.org/docs/LangRef.html) 的子集, 以便使用 LLVM 的工具链进行调试。

### Structure

相关指令在 [这里](docs/irinst.md)。
...
自然循环的相关术语详见 [LLVM Loop Terminology (and Canonical Forms)](https://llvm.org/docs/LoopTerminology.html)
...
进行基本块内的指令调度和重排，目前只有 AArch64 后端有这个功能
...
由于当时时间有限（通宵爆肝 30+ h 😭），并没有排查出具体是哪个 pass 有问题，它们在决赛时都没有开启。
